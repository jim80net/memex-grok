...
const fakeLocations = {
  toHandle: (abs: string) => `memex://test/${abs.split("/").pop()}`,
} as LocationHandleCodec;
...
describe("memex_search tool", () => {
  it("returns query_id and result list", async () => {
    ...
    const tool = makeSearchTool({ index: index as any, locations: fakeLocations, defaultTopK: 5, defaultThreshold: 0.5 });
    ...
    expect(parsed.results[0].location).toMatch(/^memex:\/\//);
