...
writeFileSync(join(config.out, "raw-manifest.md"), `# Raw evidence manifest\n\nNonce \`${config.nonce}\`; harness \`${config.harnessCommit}\`; ${files.length} files.\n\n| File | Bytes | SHA-256 |\n|---|---:|---|\n${files.map((file: any) => `| \`${file.path}\` | ${file.bytes} | \`${file.sha256}\` |`).join("\n")}\n`);
process.stdout.write(`${JSON.stringify({ nonce: config.nonce, harness_commit: config.harnessCommit, files: files.length, reviewer: "pending", passed: true }, null, 2)}\n`);
